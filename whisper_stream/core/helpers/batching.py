...
def preprocess_batch_adaptive(
    ...
    cutoff_length: int = int(chunk_length_s * target_sampling_rate)

    # For case of 1
    if len(_inputs) <= 1:
        if _inputs[0].shape[0] > cutoff_length:
            yield from tasks_callable_partial_mapping["largest"].task_callable(
                **{
                    "inputs": _inputs,
            ...
            yield from tasks_callable_partial_mapping["smallest"].task_callable(
                **{
                    "inputs": _inputs,
                    "batch_size": batch_size,
                    "target_sampling_rate": target_sampling_rate,
        ...
        asc_sorted_inputs = np.array(_inputs, dtype=object)[asc_sorted_inputs_indices]
        ...
        unchunkable_inputs: list[AudioFilesDType] = list(
            asc_sorted_inputs[:asc_sorted_split_at_index]
        )
        chunkable_inputs: list[AudioFilesDType] = list(
            asc_sorted_inputs[asc_sorted_split_at_index:]
        )
        ...
        tasks_map: dict[
            BatchedTaskProcessingStrategy, BatchPreProcessorTasksMapping
        ] = {
...
def split_array_on_primary_axis(
    arr: list[AudioFilesDType], batch_size: int, logger: BoundLogger | None = None
) -> list[list[AudioFilesDType]]:
    num_batches: int = math.ceil(len(arr) / batch_size)
    batch_length: int = len(arr) // num_batches
    splits: list[list[AudioFilesDType]] = [
        arr[i : (i + batch_length)] for i in range(0, num_batches, batch_length)
    ]
    if logger is not None:
        logger.debug(
            ...
            batch_size=batch_size,
            splits=[f"len={len(split)}" for split in splits],
        )
    return splits


def pregenerate_batching_info_for_chunkable_audio(
    chunkable_inputs: list[AudioFilesDType],
...
def iterate_batch_for_chunkable_audio(
    ...
    inputs_len: int = single_input.shape[0]
    ...
    for batch_counter, batch_segments in enumerate(batch_idx):
        ...
        chunks: list[AudioFilesDType] = [
            single_input[chunk_start:chunk_end]

...
def preprocess_batch_adaptive(
    ...
    if len(_inputs) <= 1:
        ...
        unchunkable_inputs: list[AudioFilesDType] = list(asc_sorted_inputs[:asc_sorted_split_at_index])
        chunkable_inputs: list[AudioFilesDType] = list(asc_sorted_inputs[asc_sorted_split_at_index:])
...
def split_array_on_primary_axis(
    ...
    splits: list[list[AudioFilesDType]] = [arr[i:(i+batch_length)] for i in range(0, num_batches, batch_length)]
    if logger is not None:
        logger.debug(

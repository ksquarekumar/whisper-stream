    ...
    def preprocess_batch(self, inputs, chunk_length_s=30.0, stride_length_s=None, batch_size=None):
        ...
        if isinstance(inputs, dict):
            ...
            if not ("sampling_rate" in inputs and "array" in inputs):
                msg = "When passing a dictionary to FlaxWhisperPipeline, the dict needs to contain an 'array' key containing the numpy array representing the audio, and a 'sampling_rate' key containing the sampling rate associated with the audio array."
                raise ValueError(msg)
            ...
            if in_sampling_rate != self.feature_extractor.sampling_rate:
                try:
                    ...
                    msg = "To support resampling audio files, please install 'librosa' and 'soundfile'."
        ...
        if not isinstance(inputs, np.ndarray):
            msg = f"We expect a numpy ndarray as input, got `{type(inputs)}`"
            raise ValueError(msg)
        ...
        if stride is not None:
            if stride[0] + stride[1] > inputs.shape[0]:
                msg = "Stride is too large for input"
                raise ValueError(msg)

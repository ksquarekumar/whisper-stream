...
class FlaxWhisperEncoder(nn.Module):
    ...
    def __call__(
        ...
        if input_features.shape[1:] != (self.config.num_mel_bins, self.config.max_source_positions * 2):
            msg = f"input_features.shape[1:], must be equal to (self.config.num_mel_bins, self.config.max_source_positions * 2) (got {input_features.shape[1:]}, but should be ({self.config.num_mel_bins}, {self.config.max_source_positions * 2}))"
            raise ValueError(msg)
    ...
    def decode(
        ...
        if decoder_position_ids is None:
            if past_key_values is not None:
                msg = "Make sure to provide `decoder_position_ids` when passing `past_key_values`."
                raise ValueError(msg)

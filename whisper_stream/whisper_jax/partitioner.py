...
def default_mesh(
    num_partitions: int, model_parallel_submesh: HardwareMesh | None = None, backend: str | None = None
) -> Mesh:
    ...
    if mps is None:
        msg = "No default mesh for this configuration: specify config.model_parallel_submesh explicitly."
...
class LocalChunker:
    ...
    def __init__(self, global_mesh: Mesh):
        ...
        host_location = collections.OrderedDict(
            zip(global_mesh.shape.keys(), next(iter(zip(*np.nonzero(global_mesh.devices == first_local_device)))))
        )
    ...
    def get_local_chunk_info(self, global_shape: tuple[int, ...], mesh_axes: Sequence[str | None]) -> LocalChunkInfo:
...
def standard_logical_axis_rules(
    ...
    additional_rules: LogicalAxisRules | None = None,
) -> LogicalAxisRules:
    ...
    if activation_partitioning_dims == 1 and parameter_partitioning_dims == 1:
        ...
        msg = f"`activation_partitioning_dims` = {activation_partitioning_dims} `parameter_partitioning_dims` = {parameter_partitioning_dims} is not supported."
        raise ValueError(msg)
...
class BasePartitioner(metaclass=abc.ABCMeta):
    ...
    def __init__(
        ...
        model_parallel_submesh: HardwareMesh | None = None,
        ...
        if not num_partitions and not model_parallel_submesh:
            msg = "At least one of `num_partitions` or `model_parallel_submesh` must be set."
            raise ValueError(msg)
    ...
    def get_data_layout(self, batch_size: int | None = None, host_index: int | None = None) -> DataLayout:
        ...
        if host_index is not None:
            msg = "Explicit host_index is not yet implemented."
            raise NotImplementedError(msg)
        ...
        if batch_size % mesh_size:
            msg = f"Batch size ({batch_size}) must be divisible by corresponding mesh size ({mesh_size})."
            raise ValueError(msg)
        num_shards = self._local_chunker.num_chunks[self._data_axis]
        if batch_size % num_shards:
            msg = f"Batch size ({batch_size}) must be divisible by number of replicas ({num_shards})."
            raise ValueError(msg)
...
class PjitPartitioner(BasePjitPartitioner):
    ...
    def __init__(
        ...
        params_on_devices: bool = True,
        backend: str | None = None,
        logical_axis_rules: LogicalAxisRules | None = None,
        use_cpu_pjit: bool | None = False,
    ):
    ...
    def get_mesh_axes(self, train_state: TrainState) -> TrainState:
        ...
        def _logical_to_mesh_axes(param_name, logical_axes):
            ...
            try:
                ...
                msg = f"Failed to map logical axes for {param_name}"

...
def launch_app(
    app_module_path: str,
    config: FasterWhisperAPILaunchConfig = Provide[
        CommonServiceContainer.launch_config
    ],
    logger: APIBoundLogger = Provide[CommonServiceContainer.logger],
) -> None:
    import uvicorn

    logger.info(f"Launching {app_module_path}", config=config)
    uvicorn.run(app_module_path, **config.model_dump(exclude_none=True))

...
class AsyncWhisperModel(WhisperModel):
    async def transcribe_async(
        self: WhisperModel,
        audio: BinaryIO,
        transcription_config: FasterWhisperAPITranscriptionConfig,
    ) -> tuple[Iterable[Segment], TranscriptionInfo]:
        return self.transcribe(audio=audio, **transcription_config.model_dump())


class FasterWhisperAPIModelFactory:
    async def __new__(cls) -> AsyncWhisperModel:
        instance: AsyncWhisperModel = await _faster_whisper_api_model_provider()
        return instance


@inject
async def _faster_whisper_api_model_provider(
    model_logger: APILogger = Provide[CommonServiceContainer.logger],
    ...
    audio = io.BytesIO(read_binary("faster_whisper_api.preload.data", "audio.mp3"))
...
__all__: list[str] = ["FasterWhisperAPIModelFactory"]
